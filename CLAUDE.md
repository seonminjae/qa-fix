...
# 프로젝트 재생성 (Project.swift 수정 시 반드시 실행)
tuist generate --no-open

# 앱 빌드
xcodebuild -workspace QAFixMac.xcworkspace -scheme QAFixMac \
...
xcodebuild -workspace QAFixMac.xcworkspace -scheme CLISpike \
...
**요구 도구**: Xcode 16+, Tuist 4+ (`brew install tuist` 또는 `mise install tuist`),
macOS 14+, Claude Code CLI 2.1.0+.
...
```
QAFixMac/
├── Project.swift                      # Tuist 매니페스트 (3 target: 앱 / 테스트 / CLI spike)
├── QAFixMac.xcodeproj                 # 생성물 (git ignore)
├── QAFixMac.xcworkspace                # 생성물 (git ignore)
├── Derived/                           # Tuist 캐시 (git ignore)
...
| Task | 상태 | 비고 |
|---|---|---|
| Step 0-1 스캐폴드 (Project.swift, entitlements, P0 가이드 번들) | ✅ | tuist generate로 xcodeproj/xcworkspace 생성 (구 project.yml → Tuist 4 전환 완료) |
